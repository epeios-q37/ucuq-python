...
def atkSwitch(dom, id):
  ...
  if dom.getValue(id) == "true":
    ...
    cLCD.backlightOff()
    ucuq.commit()
    dom.enableElements(BOARD_WIDGETS)
    dom.disableElements(HARDWARE_WIDGETS_WITHOUT_SWITCH)
    onDuty = True
  else:
    onDuty = False
    dom.disableElements(BOARD_WIDGETS)
