...
def lcdActiveNotesEvents_(notes, minNote, maxNote, lcd):
  counter = 0
...
def lcdDurationEvents_(duration, lcds):
  ...
  for i in range(6 * 5):
    lcds.moveTo(0,1).putString(lcds[0].getForwardPeak(i, 6 * 5))
    yield duration / (6 * 5 )
  ...
  timestamp += ucuq.dispatchEvents(
    (
      *(element for turn, voice in enumerate(tracking.voices) for element in (
        buzzerEvents_(voice, turn, prev, devices.buzzers[turn]),
        ringsActiveNotesEvents_(voice, turn, devices.rings),
        lcdActiveNotesEvents_(voice, tracking.extrema.minNotes[turn], tracking.extrema.maxNotes[turn], devices.lcds[turn])
        )
      ),
      oledEvents_(pacedNotes, tracking, devices.oleds),
      lcdTitleEvents_(PARTS_[part][0][1], maxDuration + START_DELAY_, devices.lcds),
      lcdDurationEvents_(maxDuration, devices.lcds),
      ringsRainbowEvents_(maxDuration, devices.rings)
    ),
