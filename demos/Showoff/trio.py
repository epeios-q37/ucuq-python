...
def callback_(helper, events, duration):
  ...
  for event in events:
    ...
    if event[1] != 0 and prev_[turn] == event[1]:
      buzzer.off()
    ...
    for spot in spots:
      ...
      if event[1] != 0:
        devices.rgbs.setValue(spots[indexes[turn] % len(spots)], COLORS[turn])
...
def launch(timestamp):
  ...
  devices.oleds.contrast(0).draw(PICTURE, 64, 32).show()
  
  helper = SimpleNamespace(timestamp = timestamp + 1)

  # shared.polyphonicPlay(JACQUES, 120, buzzers, callback)

  shared.polyphonicPlay(FUGUE, 160, helper, callback_)
